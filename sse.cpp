...
void yuv2rgb_with_sse(const YUV & yuv, RGB & rgb){
    ...
    __m128 * Yp32 = new __m128[yuv.size >> 2];
    __m128 * Up32 = (__m128 *)dupPU32;
    __m128 * Vp32 = (__m128 *)dupPV32;
    
    int64_t nloop = yuv.size >> 2;
    for(int i = 0; i < nloop; i++){
        Yp32[i] = _mm_set_ps((float)yuv.pY32[i << 2], (float)yuv.pY32[(i << 2) + 1],
                             (float)yuv.pY32[(i << 2) + 2], (float)yuv.pY32[(i << 2) + 3]);
...
//    rgb.write(foutcheck2);
    ...
    __m128 * srcR = new __m128[rgb.size >> 2];
    __m128 * srcG = new __m128[rgb.size >> 2];
    __m128 * srcB = new __m128[rgb.size >> 2];
    ...
    for(int i = 0; i < nloop; i++){
        ...
        srcR[i] = _mm_set_ps((float)rgb.pR32[i << 2], (float)rgb.pR32[(i << 2) + 1],
                             (float)rgb.pR32[(i << 2) + 2], (float)rgb.pR32[(i << 2) + 3]);
        srcG[i] = _mm_set_ps((float)rgb.pG32[i << 2], (float)rgb.pG32[(i << 2) + 1],
                             (float)rgb.pG32[(i << 2) + 2], (float)rgb.pG32[(i << 2) + 3]);
        srcB[i] = _mm_set_ps((float)rgb.pB32[i << 2], (float)rgb.pB32[(i << 2) + 1],
                             (float)rgb.pB32[(i << 2) + 2], (float)rgb.pB32[(i << 2) + 3]);
        ...
        dstY[i] = _mm_add_ps(dstY[i], tmp);
        dstY[i] = _mm_add_ps(dstY[i], YConst16_s);
    ...
    for(int i = 0; i < nloopUV; i++){
        ...
        dstU[i] = _mm_add_ps(dstU[i], tmp);
        dstU[i] = _mm_add_ps(dstU[i], UVConst128_s);
        ...
        dstV[i] = _mm_add_ps(dstV[i], tmp);
        dstV[i] = _mm_add_ps(dstV[i], UVConst128_s);
...
int process_with_sse(YUV &OUT_YUV, const YUV &DEM1_YUV, const YUV &DEM2_YUV, RGB &CHECK_RGB1, RGB &CHECK_RGB2, const bool mode){
    ...
    if(mode){
        cout << "Alpha Blending with dem1.yuv and dem2.yuv, output file is \"alpha2-2.yuv\":" <<endl;
        ...
        cout << "Alpha Blending with dem1.yuv, output file is \"alpha2-1.yuv\":" <<endl;
        cout << "Core function time: " << (double)total_time / CLOCKS_PER_SEC * 1000<< "ms" << endl;
        cout << "Include output time: " << (double)(clock() - begin_time) / CLOCKS_PER_SEC * 1000 << "ms" << endl << endl;
    }
